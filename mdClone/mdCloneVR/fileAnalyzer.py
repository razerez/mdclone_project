...
def add_parameter_data_to_list(data_lines):
    """
    ...
    for person in data_lines:
        person = person.split(",")
        if not any(person[curr_parameter] in sublist for sublist in parameters_list):
            parameters_list[curr_parameter].append(person[curr_parameter])
    curr_parameter += 1
...
def main(path):
    ...
    for parameter in first_line:
        add_parameter_data_to_list(data_lines)
    for person in data_lines:
        add_person_data_to_list(person, first_line)
    analyzed_file.append(parameters_list)
    analyzed_file.append(person_list)
    analyzed_file = json.dumps(analyzed_file, indent=4)
    print(analyzed_file)
